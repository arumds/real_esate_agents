...
Structure:
  ...
  4. root_agent -- the single-audience pipeline, exposed at module scope so
     `adk web`/`adk run` can find and drive it without a custom script. Note
     this only helps for interactive exploration: those tools seed session
     state from a chat message, not from a Python dict, so the
     input_record/audience/property_summary seeding `main.py` does still
     needs to happen some other way (an initial-message parser or a
     before_agent_callback) to fully work through the ADK web UI.
"""
